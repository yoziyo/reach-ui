...
From the command line in your project directory, run `npm install @yoziyo/tabs` or `yarn add @yoziyo/tabs`. Then import the components and styles that you need:

```bash
npm install @yoziyo/tabs
# or
yarn add @yoziyo/tabs
```
...
```jsx
import { TabsKeyboardActivation } from "@yoziyo/tabs";
...
```jsx
import { TabsOrientation } from "@yoziyo/tabs";
