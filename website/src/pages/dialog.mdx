...
From the command line in your project directory, run `npm install @yoziyo/dialog` or `yarn add @yoziyo/dialog`. Then import the components and styles that you need:

```bash
npm install @yoziyo/dialog
# or
yarn add @yoziyo/dialog
```
	...
	return (
		<div>
			...
			<Dialog isOpen={showDialog} onDismiss={dismiss}>
				...
				<button onClick={close}>@yoziyo/router</button>
			</Dialog>
		</div>
	);
}
