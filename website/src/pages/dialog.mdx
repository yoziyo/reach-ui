...
From the command line in your project directory, run `npm install @reach-ui-fork/dialog` or `yarn add @reach-ui-fork/dialog`. Then import the components and styles that you need:

```bash
npm install @reach-ui-fork/dialog
# or
yarn add @reach-ui-fork/dialog
```
	...
	return (
		<div>
			...
			<Dialog isOpen={showDialog} onDismiss={dismiss}>
				...
				<button onClick={close}>@reach-ui-fork/router</button>
			</Dialog>
		</div>
	);
}
