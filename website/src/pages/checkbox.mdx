...
`@reach-ui-fork/checkbox` provides two top-level components:
...
In this example, the `All fruits` checkbox is in an `indeterminate` state because some (but not all) fruits in the list are checked. While this effect is possible with plain input components, the `MixedCheckbox` component makes managing/syncing its state with the correct DOM attributes much simpler. All you have to do is pass the `checked` state, and `@reach-ui-fork/checkbox` handles the necessary aria attributes and related node data!
...
From the command line in your project directory, run `npm install @reach-ui-fork/checkbox` or `yarn add @reach-ui-fork/checkbox`. Then import the components and styles that you need:

```bash
npm install @reach-ui-fork/checkbox
# or
yarn add @reach-ui-fork/checkbox
```
...
```js
import { MixedCheckbox, useMixedCheckbox } from "@reach-ui-fork/checkbox";
```
