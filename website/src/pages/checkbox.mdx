...
`@yoziyo/checkbox` provides two top-level components:
...
In this example, the `All fruits` checkbox is in an `indeterminate` state because some (but not all) fruits in the list are checked. While this effect is possible with plain input components, the `MixedCheckbox` component makes managing/syncing its state with the correct DOM attributes much simpler. All you have to do is pass the `checked` state, and `@yoziyo/checkbox` handles the necessary aria attributes and related node data!
...
From the command line in your project directory, run `npm install @yoziyo/checkbox` or `yarn add @yoziyo/checkbox`. Then import the components and styles that you need:

```bash
npm install @yoziyo/checkbox
# or
yarn add @yoziyo/checkbox
```
...
```js
import { MixedCheckbox, useMixedCheckbox } from "@yoziyo/checkbox";
```
