...
A listbox presents a list of selectable options in a popover, which is toggled on or off by a button. Visually and behaviorally it is similar to [`@yoziyo/menu-button`](https://reach.tech/menu-button) except that `ListboxOption` components hold a value. In this sense they are more directly comparable to HTML select elements.
...
From the command line in your project directory, run `npm install @yoziyo/listbox` or `yarn add @yoziyo/listbox`. Then import the components and styles that you need:

```bash
npm install @yoziyo/listbox
# or
yarn add @yoziyo/listbox
```
