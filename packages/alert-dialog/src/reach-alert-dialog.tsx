/**
 * Welcome to @reach-ui-fork/alert-dialog!
 *
...
import type { DialogProps, DialogContentProps } from "@reach-ui-fork/dialog";
	...
	React.useEffect(() => {
		...
		if (labelId) {
			invariant(
				...
				`@reach-ui-fork/alert-dialog: You must render a \`<AlertDialogLabel>\`
          inside an \`<AlertDialog/>\`.`
			);
		}
		invariant(
			leastDestructiveRef,
			`@reach-ui-fork/alert-dialog: You must provide a \`leastDestructiveRef\` to
